...
import {
  ...
  LogOut, Settings, ImagePlus, AtSign, ShoppingBag, Mail, HelpCircle, Heart, Lock
} from 'lucide-react';
import {
...
// Subscription prices are set in ETB. Converted at ~156.5 ETB/USD (Aug 2026) — the Birr
// moves often against the dollar, so revisit this conversion periodically rather than
// treating it as fixed.
const CREATOR_PLANS = [
  { id: 'basic', name: 'Basic', price: 'ETB 150', period: '/month', features: ['Profile', 'Portfolio', 'Social links', '3 Spotlight videos'], tone: 'gray' },
  { id: 'pro', name: 'Pro', price: 'ETB 1,500', period: '/month', features: ['Priority listing', 'Analytics', 'QR card', '20 Spotlight videos'], tone: 'cyan', popular: true },
  { id: 'elite', name: 'Elite NFC', price: 'ETB 3,000', period: '/month', features: ['NFC card', 'Premium badge', 'Unlimited videos', 'Top placement'], tone: 'magenta' },
];

const BUSINESS_PLANS = [
  { id: 'starter', name: 'Starter', price: 'ETB 800', period: '/month', features: ['Profile', 'Campaign posting'], tone: 'gray' },
  { id: 'growth', name: 'Growth', price: 'ETB 3,000', period: '/month', features: ['Unlimited campaigns', 'Advanced filters', 'Analytics', 'QR card'], tone: 'cyan', popular: true },
  { id: 'enterprise', name: 'Enterprise NFC', price: 'ETB 8,000', period: '/month', features: ['NFC business card', 'Premium placement', 'Multiple team members'], tone: 'magenta' },
];
...
const CreatorCard = ({ c, saved = false, onToggleSave = () => {}, onHire = () => {} }) => (
  ...
  >
    ...
    <div className="flex items-center justify-between pt-1">
      <div>
        <p className="cm-mono text-base font-semibold" style={{ color: '#111827' }}>{c.price ? `ETB ${c.price}` : 'Rate on request'}</p>
  ...
  return (
    <div className="bg-white border rounded-2xl p-5 mb-6" style={{ borderColor: '#E5E7EB' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div>
        <div>
          <p className="text-xs font-semibold mb-2" style={{ color: '#374151' }}>Price range: ETB {filters.minPrice.toLocaleString()} – {filters.maxPrice.toLocaleString()}</p>
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-2">
              <span className="text-[11px] w-8" style={{ color: '#9CA3AF' }}>Min</span>
              <input
                type="range" min="0" max="500000" step="5000"
                value={filters.minPrice}
                onChange={e => {
                  const v = Number(e.target.value);
                  setFilters(f => ({ ...f, minPrice: Math.min(v, f.maxPrice) }));
                }}
                className="w-full"
              />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[11px] w-8" style={{ color: '#9CA3AF' }}>Max</span>
              <input
                type="range" min="0" max="500000" step="5000"
                value={filters.maxPrice}
                onChange={e => {
                  const v = Number(e.target.value);
                  setFilters(f => ({ ...f, maxPrice: Math.max(v, f.minPrice) }));
                }}
                className="w-full"
              />
            </div>
      ...
      <button
        onClick={() => setFilters({ platforms: [], city: 'All', minFollowers: 0, minPrice: 0, maxPrice: 500000, verifiedOnly: false })}
...
const Creators = ({ session, savedIds, toggleSave, onHire }) => {
  ...
  const [filters, setFilters] = useState({ platforms: [], city: 'All', minFollowers: 0, minPrice: 0, maxPrice: 500000, verifiedOnly: false });
  ...
  const filtered = allCreators.filter(c =>
    ...
    (c.price === null || (c.price >= filters.minPrice && c.price <= filters.maxPrice)) &&
    (!filters.verifiedOnly || c.verified)
  );

  const activeFilterCount = filters.platforms.length + (filters.city !== 'All' ? 1 : 0) + (filters.minFollowers > 0 ? 1 : 0) + (filters.minPrice > 0 || filters.maxPrice < 500000 ? 1 : 0) + (filters.verifiedOnly ? 1 : 0);
      ...
      <div className="bg-white border rounded-2xl p-6 md:p-8" style={{ borderColor: '#E5E7EB' }}>
        ...
        {step === 4 && (
          <div className="flex flex-col gap-5">
            ...
            <p className="text-xs -mt-3" style={{ color: '#6B7280' }}>Enter your rates in Ethiopian Birr (ETB).</p>
            <div className="grid grid-cols-2 gap-4">
...
const VISIBILITY_OPTIONS = [
  ...
  { value: 'after_message', label: 'Only people I\'ve already messaged' },
  { value: 'nobody', label: 'Nobody but me' },
];

const MESSAGING_VISIBILITY_OPTIONS = [
  { value: 'everyone', label: 'Everybody' },
  { value: 'verified_only', label: 'Verified users only' },
  { value: 'premium_only', label: 'Premium users only' },
  { value: 'nobody', label: 'No one (close new messages)' },
];

const PrivacySettings = ({ session }) => {
  const [table, setTable] = useState(null); // 'creator_profiles' | 'business_profiles'
  const [detailsVisibility, setDetailsVisibility] = useState('everyone');
  const [messagingVisibility, setMessagingVisibility] = useState('everyone');
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: cp } = await supabase
        .from('creator_profiles')
        .select('details_visibility, messaging_visibility')
        .eq('auth_user_id', session.user.id)
        .maybeSingle();
      if (cp) {
        setTable('creator_profiles');
        setDetailsVisibility(cp.details_visibility || 'everyone');
        setMessagingVisibility(cp.messaging_visibility || 'everyone');
        setLoading(false);
        return;
      }
      const { data: bp } = await supabase
        ...
        .maybeSingle();
      if (bp) {
        setTable('business_profiles');
        setDetailsVisibility(bp.details_visibility || 'everyone');
        setMessagingVisibility(bp.messaging_visibility || 'everyone');
      }
      setLoading(false);
    })();
  }, [session.user.id]);
  ...
  const save = async () => {
    ...
    const { error } = await supabase
      .from(table)
      .update({ details_visibility: detailsVisibility, messaging_visibility: messagingVisibility })
      .eq('auth_user_id', session.user.id);
    setSaving(false);
    setMessage(error ? (error.message || 'Could not save privacy settings.') : 'Privacy settings saved.');
  };
  ...
  return (
    <div className="bg-white border rounded-2xl p-6 mb-6" style={{ borderColor: '#E5E7EB' }}>
      <p className="text-sm font-semibold mb-1" style={{ color: '#111827' }}>Privacy</p>
      <p className="text-xs mb-5" style={{ color: '#6B7280' }}>Control who can see your full profile details and who can send you a first message.</p>

      <div className="mb-5">
        <label className="text-xs font-semibold block mb-1.5" style={{ color: '#374151' }}>Who can see your full profile details</label>
        <select
          value={detailsVisibility}
          onChange={e => setDetailsVisibility(e.target.value)}
        ...
        <p className="text-[11px] mt-1.5" style={{ color: '#9CA3AF' }}>
          Your name, photo, niche/industry, city, and verified badge always stay visible so people can find you.
          This controls social handles, exact pricing, portfolio links, and bio.
        </p>
      </div>

      <div className="mb-5">
        <label className="text-xs font-semibold block mb-1.5" style={{ color: '#374151' }}>Who can message you</label>
        <select
          value={messagingVisibility}
          onChange={e => setMessagingVisibility(e.target.value)}
          className="w-full border rounded-lg px-3 py-2.5 text-sm outline-none"
          style={{ borderColor: '#E5E7EB' }}
        >
          {MESSAGING_VISIBILITY_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
        <p className="text-[11px] mt-1.5" style={{ color: '#9CA3AF' }}>Existing conversations are never affected — this only applies to new messages.</p>
      </div>

      <button onClick={save} disabled={saving} style={{ background: '#111827' }} className="text-white text-sm font-semibold px-4 py-2.5 rounded-lg disabled:opacity-50">
        {saving ? 'Saving…' : 'Save privacy settings'}
      </button>
      {message && <p className="text-xs mt-3" style={{ color: message.includes('saved') ? '#0E7A3B' : '#DC2626' }}>{message}</p>}
    </div>
  );
};
  ...
  return (
    <div className="max-w-2xl mx-auto px-5 md:px-8 py-12">
      ...
      <div className="bg-white border rounded-2xl p-6 md:p-8 flex flex-col gap-5" style={{ borderColor: '#E5E7EB' }}>
        ...
        <OnboardingField label="Typical budget range" placeholder="e.g. ETB 30,000–75,000 per campaign" icon={DollarSign} value={budgetRange} onChange={e => setBudgetRange(e.target.value)} />
        <div>
...
const LockedDetailsNotice = ({ visibility }) => {
  const copy = {
    verified_only: 'This creator only shares full details with verified Commissioner accounts.',
    premium_only: 'This creator only shares full details with premium Commissioner accounts.',
    after_message: 'This creator shares full details once you\'ve exchanged a message with them.',
    nobody: 'This creator has kept these details private.',
  };
  return (
    <div className="mb-7 rounded-2xl border p-5 flex items-start gap-3" style={{ borderColor: '#E5E7EB', background: '#F8FAFC' }}>
      ...
      <div>
        <p className="text-sm font-semibold mb-1" style={{ color: '#111827' }}>Some details are private</p>
        <p className="text-xs" style={{ color: '#6B7280' }}>{copy[visibility] || 'This creator has limited who can see their full details.'}</p>
      </div>
    </div>
  );
};
...
const PublicCreatorProfile = ({ profile, canEdit = false, onEdit }) => {
  const locked = Boolean(profile.details_locked);
  const socials = !locked && profile.platforms && typeof profile.platforms === 'object' ? profile.platforms : {};
  const socialEntries = Object.entries(socials).filter(([, value]) => value && (value.handle || value.url));
  const services = !locked && profile.services && typeof profile.services === 'object' ? profile.services : {};
      ...
      <div className="max-w-4xl mx-auto px-5 md:px-8 py-8 md:py-12">
        <div className="bg-white border rounded-3xl overflow-hidden shadow-sm" style={{ borderColor: '#E5E7EB' }}>
          ...
          <div className="px-5 md:px-8 pb-8">
            ...
            {profile.bio && !locked && <p className="text-sm leading-7 mb-7" style={{ color: '#374151' }}>{profile.bio}</p>}
            {locked && <LockedDetailsNotice visibility={profile.details_visibility} />}
            ...
            {profile.portfolio_link && !locked && <a href={profile.portfolio_link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm font-semibold px-4 py-3 rounded-xl text-white" style={{ background: '#111827' }}><Globe size={15} /> View portfolio <ArrowUpRight size={15} /></a>}
          </div>
        </div>
        {canEdit && (
    ...
    <div className="max-w-4xl mx-auto px-5 md:px-8 py-8 md:py-12">
      <div className="bg-white border rounded-3xl overflow-hidden shadow-sm" style={{ borderColor: '#E5E7EB' }}>
        ...
        <div className="px-5 md:px-8 pb-8">
          ...
          {profile.bio && !locked && <p className="text-sm leading-7 mb-6" style={{ color: '#374151' }}>{profile.bio}</p>}
          {locked && <LockedDetailsNotice visibility={profile.details_visibility} />}
          <div className="mb-6"><VerificationDetails type="business" id={profile.id} /></div>
          <div className="flex flex-wrap gap-3">
            {profile.website && !locked && <a href={profile.website} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm font-semibold px-4 py-3 rounded-xl text-white" style={{ background: '#111827' }}><Globe size={15} /> Website <ArrowUpRight size={15} /></a>}
          </div>
        </div>
      </div>
...
const OfficialBusinessPage = ({ id }) => {
  const [profile, setProfile] = useState(null);
  useEffect(() => {
    supabase.rpc('get_business_profile_view', { p_id: id }).then(({ data }) => setProfile(data || null));
  }, [id]);
...
const OfficialCreatorPage = ({ id, session, setPage }) => {
  ...
  const loadProfile = async () => {
    setLoading(true);
    const { data, error } = await supabase.rpc('get_creator_profile_view', { p_id: id });
    if (!error && data) {
      setProfile(data);
