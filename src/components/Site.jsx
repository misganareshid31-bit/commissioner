...
const Footer = ({ setPage }) => (
  <footer style={{ background: '#FFFFFF' }} className="text-white mt-12">
    ...
    <div className="border-t px-5 md:px-8 py-3 flex flex-col md:flex-row justify-between gap-3" style={{ borderColor: '#FFFFFF' }}>
      <p className="text-xs" style={{ color: '#FFFFFF' }}>© 2026 Commissioner. All rights reserved.</p>
    </div>
  </footer>
);
...
const Campaigns = ({ session, setPage, appliedIds, onApply }) => (
  <div className="max-w-7xl mx-auto px-5 md:px-8 py-10">
    <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
      <div>
        <h1 className="cm-display font-bold text-2xl md:text-3xl mb-2" style={{ color: '#FFFFFF' }}>Campaigns</h1>
        <p className="text-sm" style={{ color: '#FFFFFF' }}>Open opportunities from verified businesses</p>
      </div>
      <button
    ...
    <div className="flex flex-col gap-4">
      {CAMPAIGNS.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm font-semibold mb-1" style={{ color: '#FFFFFF' }}>No campaigns posted yet</p>
          <p className="text-xs" style={{ color: '#FFFFFF' }}>Be the first business to post one, or check back soon.</p>
        </div>
      ) : CAMPAIGNS.map(camp => (
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <p className="font-semibold text-sm" style={{ color: '#FFFFFF' }}>{camp.title}</p>
                  <StatusPill status={camp.status} />
                </div>
                <p className="text-xs mb-3" style={{ color: '#FFFFFF' }}>{camp.business}</p>
                <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-xs" style={{ color: '#FFFFFF' }}>
            ...
            <div className="flex items-center gap-2 shrink-0 pl-16 md:pl-0">
              ...
              {appliedIds.includes(camp.id) ? (
                ...
                <button onClick={() => onApply(camp)} style={{ background: '#FFFFFF' }} className="text-white text-xs font-semibold px-4 py-2.5 rounded-lg">Apply</button>
              )}
            </div>
          </div>
        </div>
...
const Marketplace = ({ onMessage, session }) => {
...
const VerificationAdminQueue = () => {
  const [creators,setCreators]=useState([]); const [businesses,setBusinesses]=useState([]); const [loading,setLoading]=useState(true); const [busy,setBusy]=useState(''); const [message,setMessage]=useState(''); const [detailRow,setDetailRow]=useState(null);
  ...
  return <div className="border rounded-2xl p-5 mb-8" style={{borderColor:'#E5E7EB',background:'#FFFFFF'}}>
    ...
    <div className="space-y-3">{rows.map(r=><div key={r._type+r.id} className="border rounded-xl p-4" style={{borderColor:'#E5E7EB'}}>
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2 shrink-0">
          ...
          <button onClick={()=>setDetailRow(r)} disabled={!!busy} className="text-xs font-semibold px-3 py-2 rounded-lg border disabled:opacity-50" style={{borderColor:'#BAE6FD',color:'#036377',background:'#F0FDFF'}}><FileText size={13} className="inline mr-1"/>Brief details</button>
    ...
    {detailRow && <div className="fixed inset-0 z-[80] flex items-center justify-center p-4" style={{background:'rgba(2,8,23,.62)'}} onMouseDown={e=>{if(e.target===e.currentTarget)setDetailRow(null)}}>
      <div className="w-full max-w-2xl max-h-[88vh] overflow-y-auto rounded-2xl border bg-white shadow-2xl" style={{borderColor:'#E5E7EB'}}>
        <div className="flex items-start justify-between gap-4 p-5 border-b" style={{borderColor:'#E5E7EB'}}>
          <div><p className="text-xs font-bold uppercase tracking-wider" style={{color:'#036377'}}>Approval review</p><h3 className="cm-display text-xl font-bold mt-1" style={{color:'#111827'}}>{detailRow.name}</h3><p className="text-xs mt-1" style={{color:'#6B7280'}}>{detailRow._type==='creator'?'Creator':'Business'} · Submitted {new Date(detailRow.created_at).toLocaleDateString()}</p></div>
          <button onClick={()=>setDetailRow(null)} className="p-2 rounded-lg border" style={{borderColor:'#E5E7EB',color:'#374151'}} aria-label="Close"><X size={17}/></button>
        </div>
        <div className="p-5 space-y-5">
          <div className="flex items-center gap-3"><Avatar name={detailRow.name} size={56} src={detailRow.profile?.avatar_url}/><div><p className="text-sm font-semibold" style={{color:'#111827'}}>{detailRow.profile?.username?`@${detailRow.profile.username}`:'No username'}</p><p className="text-xs" style={{color:'#6B7280'}}>{detailRow.profile?.city||'Location not provided'}{detailRow.profile?.primary_niche?` · ${detailRow.profile.primary_niche}`:''}{detailRow.profile?.industry?` · ${detailRow.profile.industry}`:''}</p></div></div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <div className="rounded-xl p-3" style={{background:'#F9FAFB'}}><p className="text-[10px] uppercase font-bold" style={{color:'#6B7280'}}>Request</p><p className="text-xs font-semibold mt-1 capitalize" style={{color:'#111827'}}>{detailRow.status.replace('_',' ')}</p></div>
            <div className="rounded-xl p-3" style={{background:'#F9FAFB'}}><p className="text-[10px] uppercase font-bold" style={{color:'#6B7280'}}>Approved</p><p className="text-xs font-semibold mt-1" style={{color:detailRow.profile?.approved?'#0E7A3B':'#9A4A0C'}}>{detailRow.profile?.approved?'Yes':'No'}</p></div>
            <div className="rounded-xl p-3" style={{background:'#F9FAFB'}}><p className="text-[10px] uppercase font-bold" style={{color:'#6B7280'}}>Verified</p><p className="text-xs font-semibold mt-1" style={{color:detailRow.profile?.verified?'#0E7A3B':'#9A4A0C'}}>{detailRow.profile?.verified?'Yes':'Verification required'}</p></div>
            <div className="rounded-xl p-3" style={{background:'#F9FAFB'}}><p className="text-[10px] uppercase font-bold" style={{color:'#6B7280'}}>Plan</p><p className="text-xs font-semibold mt-1 capitalize" style={{color:'#111827'}}>{detailRow.profile?.plan||'basic'}</p></div>
          </div>
          <div><p className="text-xs font-bold uppercase tracking-wide mb-2" style={{color:'#374151'}}>Verification information</p><div className="flex flex-wrap gap-2">{detailRow._type==='creator'?<><TrustChip tone={detailRow.identity_status==='verified'?'green':'gray'}>{detailRow.identity_status==='verified'?'Identity checked':'Identity not checked'}</TrustChip><TrustChip tone={detailRow.account_status==='verified'?'green':'gray'}>{detailRow.account_status==='verified'?'Account ownership checked':'Account ownership pending'}</TrustChip><TrustChip tone={detailRow.followers_status==='verified'?'green':'gray'}>{detailRow.followers_status==='verified'?'Followers checked':'Follower count not verified'}</TrustChip><TrustChip tone={detailRow.engagement_status==='verified'?'green':'gray'}>{detailRow.engagement_status==='verified'?'Engagement checked':'Engagement not verified'}</TrustChip></>:<><TrustChip tone={detailRow.registration_status==='verified'?'green':'gray'}>{detailRow.registration_status==='verified'?'Registration checked':'Registration not verified'}</TrustChip><TrustChip tone={detailRow.license_status==='verified'?'green':'gray'}>{detailRow.license_status==='verified'?'License checked':'License pending'}</TrustChip><TrustChip tone={detailRow.representative_status==='verified'?'green':'gray'}>{detailRow.representative_status==='verified'?'Representative checked':'Representative pending'}</TrustChip></>}</div></div>
          {detailRow._type==='creator' && <div className="grid grid-cols-2 gap-3"><div><p className="text-[11px] font-semibold" style={{color:'#6B7280'}}>Audience</p><p className="text-sm font-semibold" style={{color:'#111827'}}>{detailRow.profile?.audience||'Not provided'}</p></div><div><p className="text-[11px] font-semibold" style={{color:'#6B7280'}}>Platforms</p><p className="text-sm font-semibold" style={{color:'#111827'}}>{Array.isArray(detailRow.profile?.platforms)?detailRow.profile.platforms.join(', '):(detailRow.profile?.platforms||'Not provided')}</p></div></div>}
          <div><p className="text-xs font-bold uppercase tracking-wide mb-2" style={{color:'#374151'}}>Applicant's brief note</p><div className="rounded-xl border p-3 text-sm leading-6" style={{borderColor:'#E5E7EB',background:'#F9FAFB',color:'#374151'}}>{detailRow.evidence_note?.trim()||'No additional note was provided.'}</div></div>
        </div>
        <div className="flex justify-end gap-2 p-5 border-t" style={{borderColor:'#E5E7EB'}}><button onClick={()=>setDetailRow(null)} className="px-4 py-2 rounded-lg border text-sm font-semibold" style={{borderColor:'#D1D5DB',color:'#374151'}}>Close</button>{!detailRow.profile?.approved&&detailRow.status!=='deleted'&&<button onClick={async()=>{setDetailRow(null);await act(detailRow._type,detailRow,'approve')}} className="px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{background:'#0E7A3B'}}>Approve</button>}</div>
      </div>
    </div>}
