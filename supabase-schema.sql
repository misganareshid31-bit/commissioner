...
-- Admin-only page deletion. This must be a SECURITY DEFINER RPC because
-- the normal profile RLS policies intentionally do not allow arbitrary
-- deletes from the browser. It deletes only the selected creator/business
-- profile row and never deletes the user's auth account.
...
begin
  ...
  if p_kind = 'creator' then
    delete from public.creator_profiles where id = p_page_id;
  elsif p_kind = 'business' then
    delete from public.business_profiles where id = p_page_id;
  else
    raise exception 'invalid page kind';
  end if;

  if found then
    return jsonb_build_object('deleted', true, 'kind', p_kind, 'id', p_page_id);
  end if;

  return jsonb_build_object('deleted', false, 'kind', p_kind, 'id', p_page_id);
end;
