-- Commissioner Marketplace Messaging - FINAL, self-contained/idempotent
-- Run this in Supabase SQL Editor. It installs the messaging tables/functions
-- required by the Marketplace Message button and listing inquiries.
...
create index if not exists messages_sender_read_idx on public.messages(sender_id, read_at) where read_at is null;
...
-- General conversation starter. Marketplace uses its own function below so it
-- does not depend on profile-completion/network-gate functions.
...
begin
  ...
  from (
    select c.id, other.user_id as other_user_id,
...
begin
  if not exists(select 1 from public.conversation_members where conversation_id=p_conversation_id and user_id=auth.uid()) then raise exception 'not a conversation member'; end if;
...
create policy "Sellers view marketplace inquiries" on public.marketplace_inquiries for select to authenticated using(seller_user_id=auth.uid());
drop policy if exists "Buyers create marketplace inquiries" on public.marketplace_inquiries;
create policy "Buyers create marketplace inquiries" on public.marketplace_inquiries for insert to authenticated with check(buyer_user_id=auth.uid());

-- Marketplace messaging deliberately bypasses profile completion and network launch gates.
